...
export const YesNoDemo = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const { member } = Member.newMember<Uint8Array>(eciesService, MemberType.System, 'Electoral Commission', new EmailString('commission@gov.uk'));
        await member.deriveVotingKeys();
        setAuthority(member as Member<Uint8Array>);
        const newPoll = PollFactory.create<Uint8Array>(['Yes', 'No'], 'yes-no' as any, member as Member<Uint8Array>);
        setPoll(newPoll);
      } catch (e) {
  ...
  const reset = () => {
    if (!authority) return;
    const newPoll = PollFactory.create<Uint8Array>(['Yes', 'No'], 'yes-no' as any, authority);
    setPoll(newPoll);
    setVotes(new Map());
    setResults(null);
  };
