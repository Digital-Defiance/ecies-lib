...
describe('InvariantValidator', () => {
  ...
  describe('Default Invariants', () => {
    ...
    it('should validate RecipientIdConsistency', () => {
      const result = InvariantValidator.checkInvariant(Constants, 'RecipientIdConsistency');
      expect(result).toBe(true);
    });

    it('should validate Pbkdf2ProfilesValidity', () => {
      const result = InvariantValidator.checkInvariant(Constants, 'Pbkdf2ProfilesValidity');
      expect(result).toBe(true);
    });

    it('should validate EncryptionAlgorithmConsistency', () => {
      const result = InvariantValidator.checkInvariant(Constants, 'EncryptionAlgorithmConsistency');
      expect(result).toBe(true);
    });
  });
  ...
  describe('RecipientIdConsistency Invariant', () => {
    it('should catch mismatched MEMBER_ID_LENGTH', () => {
      const badConfig = createMutableConfig(createRuntimeConfiguration({ idProvider: new GuidV4Provider() }));
      // Manually break the config to test validation
      (badConfig as any).MEMBER_ID_LENGTH = 999;

      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/RecipientIdConsistency/);
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/MEMBER_ID_LENGTH/);
    });

    it('should catch mismatched ECIES.MULTIPLE.RECIPIENT_ID_SIZE', () => {
      const badConfig = createMutableConfig(createRuntimeConfiguration({ idProvider: new ObjectIdProvider() }));
      (badConfig as any).ECIES.MULTIPLE.RECIPIENT_ID_SIZE = 999;

      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/RecipientIdConsistency/);
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/RECIPIENT_ID_SIZE/);
    });

    it('should pass for ObjectID provider (12 bytes)', () => {
      const config = createRuntimeConfiguration({ idProvider: new ObjectIdProvider() });
  ...
  describe('Pbkdf2ProfilesValidity Invariant', () => {
    it('should catch iterations too low', () => {
      ...
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/Pbkdf2ProfilesValidity/);
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/iterations/);
    });

    it('should catch iterations too high', () => {
      const badConfig = createMutableConfig(createRuntimeConfiguration());
      (badConfig as any).PBKDF2_PROFILES.BROWSER_PASSWORD.iterations = 20_000_000; // Too high
    ...
    it('should catch saltBytes too small', () => {
      ...
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/Pbkdf2ProfilesValidity/);
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/saltBytes/);
    });
    ...
    it('should catch hashBytes invalid', () => {
      ...
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/Pbkdf2ProfilesValidity/);
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/hashBytes/);
    });
  });
  ...
  describe('EncryptionAlgorithmConsistency Invariant', () => {
    it('should catch wrong symmetric key size', () => {
      ...
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/EncryptionAlgorithmConsistency/);
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/KEY_SIZE/);
    });
    ...
    it('should catch unsupported curve', () => {
      ...
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/EncryptionAlgorithmConsistency/);
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/CURVE_NAME/);
    });
    ...
    it('should catch invalid key derivation path', () => {
      ...
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/EncryptionAlgorithmConsistency/);
      expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/PRIMARY_KEY_DERIVATION_PATH/);
    });
  });
  ...
  describe('Custom Invariants', () => {
    ...
    it('should validate custom invariants', () => {
      ...
      try {
        expect(() => InvariantValidator.validateAll(badConfig)).toThrow(/CustomTest/);
      } catch (e) {
        console.error('Second expect failed:', e);
        console.error('badConfig.PBKDF2.SALT_BYTES:', (badConfig as any).PBKDF2.SALT_BYTES);
        throw e;
      }
    });
  ...
  describe('Failure Details', () => {
    it('should return null for passing invariant', () => {
      const details = InvariantValidator.getFailureDetails(Constants, 'RecipientIdConsistency');
    ...
    it('should return error message for failing invariant', () => {
      ...
      const details = InvariantValidator.getFailureDetails(badConfig, 'RecipientIdConsistency');
