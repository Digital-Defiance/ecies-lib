...
describe('EciesFileService (Frontend)', () => {
  ...
  beforeAll(() => {
    ...
    const publicKeyBytes = new Uint8Array(wallet.getPublicKey());
    const combinedPublicKey = new Uint8Array(1 + publicKeyBytes.length);
    combinedPublicKey[0] = 0x04;
    combinedPublicKey.set(publicKeyBytes, 1);
    recipientPublicKey = combinedPublicKey;
