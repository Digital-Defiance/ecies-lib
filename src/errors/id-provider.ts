...
import { HandleableErrorOptions, TypedHandleableError, buildReasonMap } from '@digitaldefiance/i18n-lib';
 ...
 */
export class IdProviderError extends TypedHandleableError<typeof IdProviderErrorType, EciesStringKey> {
  /**
   * Reason map cache to avoid rebuilding on every error instantiation.
   */
  private static readonly REASON_MAP = buildReasonMap<typeof IdProviderErrorType, EciesStringKey>(
    IdProviderErrorType,
    ['Error', 'IdProviderError']
  );
  ...
  constructor(
    ...
    super(EciesComponentId, type, IdProviderError.REASON_MAP, source, options, language, templateParams);
